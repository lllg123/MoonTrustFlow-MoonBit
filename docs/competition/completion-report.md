...
- 新增风险评估、图查询与导出、策略审计、SARIF、批量分析、基线比较、质量门禁和模型契约，
  使核心实现达到 3048 行生产 MoonBit 源码。
- 当前 MoonBit 0.10.3 本地 `moon check --target all --deny-warn` 已通过；Wasm、Wasm-GC
  和 JS 三个目标的 26 个测试全部通过。native 目标由三平台 CI 覆盖。
...
- 本次 Mooncakes 发布使用当前 GitHub 账号 `lllg123` 对应的命名空间 `lllg123/moontrustflow`；历史维护身份仅用于解释既有提交记录，不作为新的包发布身份。
